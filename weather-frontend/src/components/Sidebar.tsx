...
export type AppPage = 'starred' | 'maps' | 'community' | 'analytics' | 'account'
...
const navigation: ReadonlyArray<{
    ...
    { label: 'Analytics', href: '/analytics', icon: 'analytics', page: 'analytics' },
...
export function Sidebar({ currentPage }: SidebarProps) {
    ...
    return (
        <aside className="app-navigation liquid-glass fixed top-4 bottom-4 left-4 z-[800] isolate flex w-24 min-w-0 flex-col items-center overflow-hidden rounded-3xl px-2.5 pt-4 pb-3 max-[1025px]:top-auto max-[1025px]:right-3 max-[1025px]:left-3 max-[1025px]:w-auto max-[1025px]:flex-row max-[1025px]:items-center max-[1025px]:gap-1.5 max-[1025px]:rounded-[23px] max-[1025px]:p-2 max-[350px]:gap-0.5 max-[350px]:p-1">
            <Link
            ...
            <nav className="relative z-[1] mt-6 min-h-0 flex-1 w-full overflow-x-hidden overflow-y-auto overscroll-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden max-[1025px]:m-0 max-[1025px]:overflow-visible" aria-label="Primary navigation">
                <ul className="m-0 grid w-full min-w-0 list-none gap-2.5 p-0 max-[1025px]:grid-cols-5 max-[1025px]:gap-1 max-[430px]:gap-0.5">
                    {navigation.map((item) => {
                        ...
                        return (
                            <li className="min-w-0" key={item.page}>
                                <NavLink
                                    className={`flex min-h-[64px] w-full min-w-0 flex-col items-center justify-center gap-1 rounded-2xl border px-1.5 py-2 text-[0.68rem] font-extrabold no-underline transition-all duration-200 active:scale-95 focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#1596f566] motion-reduce:transition-none max-[1025px]:min-h-14 max-[1025px]:rounded-[17px] max-[1025px]:p-1 max-[1025px]:text-[0.64rem] max-[430px]:px-0.5 max-[430px]:text-[0.58rem] max-[350px]:text-[0.55rem] ${isCurrent ? 'border-[var(--active-border)] bg-[var(--active-surface)] text-[var(--active-ink)] shadow-sm' : 'border-transparent text-[var(--muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--ink)]'}`}
                                    to={item.href}
